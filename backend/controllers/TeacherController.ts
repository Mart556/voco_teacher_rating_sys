...
import TeacherService, { TeacherServiceError } from "../services/TeacherService";
...
export const createTeacher = async (req: Request, res: Response) => {
    try {
        ...
        if (error instanceof TeacherServiceError) {
            ...
            if (error.code === "DUPLICATE_TEACHER") return res.status(409).json({ error: error.message });
        }
        res.status(500).json({ error: "Serveri viga" });
    }
};
...
export const getTeacherById = async (req: Request, res: Response) => {
    try {
        ...
        if (error instanceof TeacherServiceError && error.code === "TEACHER_NOT_FOUND") {
